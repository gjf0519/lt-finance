...
public class StockAlgorithm {
    ...
    public static List<Double> calculate(List<Double> prices,int day){
        ...
        for(int i = (day-1);i < prices.size();i++){
            ...
            for(int y = (i-1);y >= stopy;y--){
                avg = MathUtil.add(avg,prices.get(y),4);
            }
            avg = MathUtil.div(avg,day,2);
            result.add(avg);
        }
        return result;
    }
    ...
    public static String calculateForm(Double high,Double low,Double open,Double close){
        ...
        double ud = dlen - ulen;
        //十字星
        double doji = MathUtil.div(open,close,2);
        //整体长度
        ...
        if(0 == klen){
            ...
            bratio = MathUtil.div(blen,klen,2);
        }
        //身体长度占比
        String kform = "";
        if(ulen == 0){
    ...
    public static void main(String[] args) {
        double an = MathUtil.div(calculateAngle(5.85,5.67),8);
